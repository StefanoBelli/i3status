...
void print_memory_info(yajl_gen json_gen, char *buffer, const char* format) {
    ...
    for (; *format; format++) {
        ...
        if (BEGINS_WITH(format + 1, "avail")) {
            outwalk += sprintf(outwalk, "%3.2f", GBYTE(current.freeram));
            format += 5;
        } else if(BEGINS_WITH(format + 1, "total")) {
            outwalk += sprintf(outwalk, "%3.2f", GBYTE(current.totalram));
            format += 5;
        }  else if(BEGINS_WITH(format + 1, "used")) {
            outwalk += sprintf(outwalk, "%3.2f", GBYTE(current.totalram - current.freeram));
            format += 4;
        } 
    }
